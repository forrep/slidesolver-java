...
import jp.ne.raccoon.slidesolver.Solver.FieldCondition.ScoringType;
import jp.ne.raccoon.slidesolver.Solver.FieldOperation.FieldComparator;
...
public class Solver {
	...
	private static boolean FIELD_OPERATION_POOL_ENABLED = false;
...
//		new SolverConfiguration(FieldComparator.comparatorDefault, 240, 2400, ScoringType.LEFT, 0),
	...
	private static FieldOperation solve(SolverConfiguration configuration, FieldOperation initialField, FieldHistory fieldHistoryAsc, FieldHistory fieldHistoryDsc) {
			...
			outer : for (FieldOperation field: setAsc) {
				for (Direction direction : Direction.values()) {
					...
					if (direction.reverse() != field.lastOperation && field.canMove(direction)) {
						...
						fieldTemp = new Field(newField);
						// 盤面のループ判定、過去に登場した盤面の場合はそれ以降の解析を行わない
						if (!fieldHistoryAsc.add(fieldTemp)) {
						...
						if (fieldHistoryDsc.contains(fieldTemp, false)) {
							foundAnswerAsc = fieldTemp;
							foundAnswerDsc = fieldHistoryDsc.get(fieldTemp, false);
							break outer;
						}
					}
				}
			...
			outer : for (FieldOperation field: setDsc) {
				for (Direction direction : Direction.values()) {
					...
					if (direction.reverse() != field.lastOperation && field.canMove(direction)) {
						...
						fieldTemp = new Field(newField);
						// 盤面のループ判定、過去に登場した盤面の場合はそれ以降の解析を行わない
						if (!fieldHistoryDsc.add(fieldTemp)) {
							// 他方向の移動も近似盤面となるので解析を中止する
							break;
						}
						...
						if (fieldHistoryAsc.contains(fieldTemp, false)) {
							foundAnswerDsc = fieldTemp;
							foundAnswerAsc = fieldHistoryAsc.get(fieldTemp, false);
							break outer;
						}
					}
				}
		...
		if (foundAnswerAsc != null || foundAnswerDsc != null) {
			...
			if (foundAnswerAsc != null) {
				answerOperation.move(foundAnswerAsc.getOperations(false));
			}
			if (foundAnswerDsc != null) {
				answerOperation.move(foundAnswerDsc.getOperations(true));
			}
	...
	public static class Field {
		...
		public String getOperations(boolean reverseMode) {
			return FieldOperation.getOperations(operations, currentOperation, operationCount, reverseMode);
		}
	}
	...
	public static class FieldHistory {
		...
		public FieldHistory(int capacity) {
			table = new Entry[Integer.highestOneBit((int)((float) capacity * 1.5f)) << 1];
		...
		public boolean add(Field field) {
			Entry entry = getEntry(field, false);
			if (entry != null) {
			...
			int bucket = indexFor(field.hashCode);
			recycle.entry = field;
			recycle.next = table[bucket];
			recycle.version = version;
			table[bucket] = recycle;
			list[index] = recycle;
			
			// ARM向け最適化、剰余演算は行わない
			// 元コード index = (index + 1) % list.length;
			++index;
		...
		public Field get(Field field, boolean current) {
			Entry entry = getEntry(field, current);
			return entry != null ? entry.entry : null;
		}
		private Entry getEntry(Field field, boolean current) {
			for (Entry candidate = table[indexFor(field.hashCode)]; candidate != null; candidate = candidate.next) {
				if (candidate.entry.equals(field)) {
					return !current || candidate.version == version ? candidate : null;
				}
			}
			return null;
		}
		public boolean contains(Field field, boolean current) {
			return get(field, current) != null;
		}
		/**
		 * すべてのデータを論理削除
		...
		public void clear() {
			++version;
			if (version == 0) {
				// Integer.MAX_VALUEがオーバーフローした場合はすべてのEntryのバージョンを0にリセットしてversionを1にする
		...
		private void remove(Entry field) {
			int bucket = indexFor(field.entry.hashCode);
			Entry candidate = table[bucket];
			Entry prevCandidate = null;
			while (candidate != null) {
				if (candidate == field) {
					if (prevCandidate == null) {
						table[bucket] = candidate.next;
					}
					else {
